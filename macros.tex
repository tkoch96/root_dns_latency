...
%consistency
\newcommand\tOne{Tier-1 \ac{isp}\xspace}
\newcommand\tOnes{Tier-1 \acp{isp}\xspace}
\newcommand\tTwo{Tier-2 \ac{isp}\xspace}
\newcommand\tTwos{Tier-2 \acp{isp}\xspace}
\newcommand\pfr{provider-free reachability\xspace}

%because I want this functionality
\newcommand\aclpi[1]{\textit{\aclp{#1}}~(\acsp{#1})\acused{#1}\xspace}
\newcommand\acspu[1]{\acsp{#1}\acused{#1}\xspace}

\definecolor{cy}{rgb}{0.66, 0.66, 0.66}
\newcommand\twa[1]{\textit{\textcolor{cy}{[todd] #1}}}
...
% Anonymized Commands
\newcommand\ISIone{a research lab in a university in America\xspace} % introductory reference
\newcommand\ISItwo{the research lab} % more general references